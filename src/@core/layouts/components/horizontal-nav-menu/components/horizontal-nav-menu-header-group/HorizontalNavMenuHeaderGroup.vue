<template>
    ...
    >
        <template #activator="{ on, attrs }">
            <v-btn
                ref="refActivator"
                :color="isActive ? '#E7F3FF' : 'transparent'"
                depressed
                large
                rounded
                v-bind="attrs"
                class="text-capitalize text-base font-weight-regular px-6"
                :class="[{ 'group-activator-active': isActive }, { 'menu-open': isMenuActive }]"
                v-on="on"
            >
                ...
                <v-icon size="20" color="#818B9C" class="ml-1">
                    {{ icons.mdiChevronDown }}
                </v-icon>
            </v-btn>
        </template>
        <v-list ref="refContent" :elevation="$vuetify.theme.dark ? 9 : 8" class="menu-list-group">
