...
import { computed, inject, nextTick, ref, watch, watchEffect } from '@vue/composition-api'
// eslint-disable-next-line object-curly-newline
import { useUtils as useAclUtils } from '@core/libs/acl'
import { useUtils } from '@core/libs/i18n'
import themeConfig from '@themeConfig'
import { invoke, until, useMouseInElement, useWindowSize } from '@vueuse/core'
...
export default {
    ...
    setup(props) {
        // eslint-disable-next-line object-curly-newline
        const { isActive, isOpen, updateGroupOpen, updateIsActive } = useHorizontalNavMenuGroup(
            props.item,
        )
        ...
        const isMenuOpen = computed(
            () =>
                !(isMouseOutsideOfContent.value && isMouseOutsideOfActivator.value) ||
                ...
                nextTick(() => {
                    setTimeout(() => {
                        ...
                        const { left: childDropdownLeft } =
                            refContent.value.$el.getBoundingClientRect()
                        const shallDropLeft =
